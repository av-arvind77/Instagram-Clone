package com.quickblox.sample.video.activities;
...
import com.quickblox.sample.video.R;
import com.quickblox.sample.video.db.QbUsersDbManager;
import com.quickblox.sample.video.fragments.AudioConversationFragment;
import com.quickblox.sample.video.fragments.BaseConversationFragment;
import com.quickblox.sample.video.fragments.ConversationFragmentCallbackListener;
import com.quickblox.sample.video.fragments.IncomeCallFragment;
import com.quickblox.sample.video.fragments.IncomeCallFragmentCallbackListener;
import com.quickblox.sample.video.fragments.OnCallEventsController;
import com.quickblox.sample.video.fragments.ScreenShareFragment;
import com.quickblox.sample.video.fragments.VideoConversationFragment;
import com.quickblox.sample.video.util.NetworkConnectionChecker;
import com.quickblox.sample.video.utils.Consts;
import com.quickblox.sample.video.utils.FragmentExecuotr;
import com.quickblox.sample.video.utils.PermissionsChecker;
import com.quickblox.sample.video.utils.QBEntityCallbackImpl;
import com.quickblox.sample.video.utils.RingtonePlayer;
import com.quickblox.sample.video.utils.SettingsUtil;
import com.quickblox.sample.video.utils.UsersUtils;
...
public class CallActivity extends BaseActivity implements QBRTCClientSessionCallbacks, QBRTCSessionStateCallback, QBRTCSignalingCallback,
    ...
    public static String userOtherName = "";

    public static void start(Context context,
                             boolean isIncomingCall, String userOtherName) {

        Intent intent = new Intent(context, CallActivity.class);
        intent.putExtra(Consts.EXTRA_IS_INCOMING_CALL, isIncomingCall);
        intent.putExtra(Consts.EXTRA_OTHER_NAME, userOtherName);
...
//            we have already currentSession == null, so it's no reason to do further initialization
    ...
    private void startScreenSharing(final Intent data){
        ...
        FragmentExecuotr.addFragmentWithBackStack(getSupportFragmentManager(), R.id.frame_content, screenShareFragment, ScreenShareFragment.TAG);
    ...
    private void parseIntentExtras() {
        ...
        userOtherName = getIntent().getExtras().getString(Consts.EXTRA_OTHER_NAME);
    }
            ...
            public void run() {
                if (show) {
                    ...
                    if (connectionView.getParent() == null) {
                        ((ViewGroup) CallActivity.this.findViewById(R.id.frame_content)).addView(connectionView);
                    }
                } else {
                    ((ViewGroup) CallActivity.this.findViewById(R.id.frame_content)).removeView(connectionView);
                }
            }
        });
...
//            showToast("User " + participantName + " " + getString(R.string.text_status_hang_up) + " conversation");
    ...
    private void addIncomeCallFragment() {
        ...
        if (currentSession != null) {
            ...
            FragmentExecuotr.addFragment(getSupportFragmentManager(), R.id.frame_content, fragment, INCOME_CALL_FRAGMENT);
        } else {
    ...
    private void addConversationFragment(boolean isIncomingCall) {
        BaseConversationFragment conversationFragment = BaseConversationFragment.newInstance(
                ...
                isIncomingCall);
        FragmentExecuotr.addFragment(getSupportFragmentManager(), R.id.frame_content, conversationFragment, conversationFragment.getClass().getSimpleName());
    }
