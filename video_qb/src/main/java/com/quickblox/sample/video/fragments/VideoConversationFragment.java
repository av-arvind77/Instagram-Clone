...
import com.quickblox.sample.video.adapters.OpponentsFromCallAdapter;
...
//          for correct removing item in adapter
        ...
        timerChronometer = (Chronometer) view.findViewById(R.id.timer_chronometer);
...
//      get opponentVideoTrack - opponent's video track from recyclerView
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
     if(item.getItemId()==R.id.camera_switch){
         switchCamera(item);
     }else  if(item.getItemId()==R.id.screen_share){
         startScreenSharing();
     }
        return true;
    }
